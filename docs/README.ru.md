...
| `secret.pod-reloader.deckhouse.io/auto`      | Deployment, DaemonSet, StatefulSet | При изменениях только в связанных Secret'ах произойдёт перезапуск подов этого контроллера | `"true"`, `"false"` |
| `configmap.pod-reloader.deckhouse.io/auto`   | Deployment, DaemonSet, StatefulSet | При изменениях только в связанных ConfigMap'ах произойдёт перезапуск подов этого контроллера | `"true"`, `"false"` |
